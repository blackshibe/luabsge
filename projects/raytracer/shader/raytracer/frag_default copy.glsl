...
// Function to get a mesh triangle at a specific index
MeshTriangle getMeshTriangle(int triangleIndex) {
    // Each triangle occupies 3 texels (vec3 = 3 floats, but we use vec4 texels)
    int baseIndex = triangleIndex * 3;
    
    MeshTriangle tri;
    tri.p1 = texelFetch(mesh_triangle_data, baseIndex).xyz;
    tri.p2 = texelFetch(mesh_triangle_data, baseIndex + 1).xyz;
    tri.p3 = texelFetch(mesh_triangle_data, baseIndex + 2).xyz;
    
    return tri;
}

// Function to get mesh metadata at a specific index
MeshMetadata getMeshMetadata(int meshIndex) {
    // Each mesh metadata occupies 5 texels (mat4 = 4 vec4, int = 1 float)
    int baseIndex = meshIndex * 5;
    
    MeshMetadata meta;
    
    // Reconstruct mat4 from 4 texels
    vec4 col0 = texelFetch(mesh_data, baseIndex);
    vec4 col1 = texelFetch(mesh_data, baseIndex + 1);
    vec4 col2 = texelFetch(mesh_data, baseIndex + 2);
    vec4 col3 = texelFetch(mesh_data, baseIndex + 3);
    meta.matrix = mat4(col0, col1, col2, col3);
    
    // Get triangle count (stored as float, convert back to int)
    meta.triangles = floatBitsToInt(texelFetch(mesh_data, baseIndex + 4).r);
