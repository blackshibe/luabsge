...
vec3 uniformSampleHemisphere(vec3 n, inout uint seed) {
    vec2 u = RandomVec2(seed);
...
vec3 PathTrace(Ray initial_ray, inout uint random_seed) {
    vec3 color = vec3(0.0);
...
void main() {
    ...
    for (int sample = 0; sample < sample_count; sample++) {
        ...
        uint random_seed = uint(blue_noise * 4294967295.0) + uint(sample * 12345);
